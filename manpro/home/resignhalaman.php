...
function getdata($con) {
    $output = "";
    $count = 0;
    $query = "SELECT id,NIK, Nama , Tgl_Lahir ,Alamat, Agama, Status_Kawin, Email, No_Telp, CV, Photo_Profile, Tgl_Resign, Alasan FROM `resign`";
    ...
    while($row = mysqli_fetch_array($result)) {
        ...
        <td style="cursor:pointer; id="'.$row['id'].'"  onclick="liat('.$row['id'].')">'.$row['Nama'].'</td>
        <td>'.$row['NIK'].'</td>
        <td>'.$row['Alamat'].'</td>
        <td>'.$row['Tgl_Lahir'].'</td>
