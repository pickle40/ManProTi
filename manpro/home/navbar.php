<nav class="navbar navbar-expand-lg navbar-dark shadow-sm" style="background-color: black">
        <div class="container">
        <a class="navbar-brand" href="#">
          <img src="kucing.jpg" width="30" height="30" class="d-inline-block align-top" alt="">
          Kocheng
        </a>
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
          <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarNav" style="margin-left:700px;">
          <ul class="navbar-nav"  style="margin-left: auto;">
            <li class="nav-item">
              <a class="nav-link" href="listpegawai.php">Pegawai</a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="listlamaran.php">Lamaran</a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="absensi.php">Absensi</a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="resignhalaman.php">Resign</a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="halaman_history.php">History</a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="pelatihan_admin.php">Pelatihan</a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="../logout.php">Logout</a>
            </li>
          </ul>
        </div>
        </div>
      </nav>