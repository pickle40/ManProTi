...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	if (empty($_POST['username'])) {
		...
		$check = "SELECT `username`, `password`, `id` FROM `user` WHERE username='".$username."'";
		$result = $link->query($check);
		$count = $result->num_rows;
				if($count > 0){
					while($row = $result->fetch_assoc()){
						if($hashpassword == $row["password"]){
							$role = mysqli_fetch_assoc(mysqli_query($link, "SELECT status,name,username, id FROM user WHERE username = '$username'"));
							...
							{
								header("Location:home/indexstaff.php");
							}
							else
							{
